...
struct RateTableTests {
    ...
    private let table = RateTable([
        ExchangeRateDataValue(currencyCode: "EUR", rate: usdToEUR),
        ExchangeRateDataValue(currencyCode: "GBP", rate: usdToGBP),
        ExchangeRateDataValue(currencyCode: "JPY", rate: usdToJPY),
    ])
    ...
    func zeroTargetRateNonUsdBase() {
        let table = RateTable([
            ...
            ExchangeRateDataValue(currencyCode: "GBP", rate: 0.0),
        ])
    ...
    func historicalPointsTable() {
        let historical = RateTable(points: [HistoricalRateDataPointValue(currencyCode: "EUR", rate: 1.5)])
    ...
    func decimalConversionZeroBase() {
        let table = RateTable([
            ExchangeRateDataValue(currencyCode: "JPY", rate: 0),
            ExchangeRateDataValue(currencyCode: "USD", rate: 1),
        ])
    ...
    func extremeMagnitudeDivision() {
        let table = RateTable([
            ExchangeRateDataValue(currencyCode: "BIG", rate: 1_000_000_000.0),
            ExchangeRateDataValue(currencyCode: "TNY", rate: 0.000_000_001),
        ])
